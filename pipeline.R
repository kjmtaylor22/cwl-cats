...
dysb.5 <- facet.updown(comm5[,asvs], tax, meta, "BodySite", "Infection", "Mock", # draw plot and output statistics for 5dpi data
  ...
  group_by(BodySite, Infection) %>% summarize_at(vars(Log2.Abundance), funs(mean)) %>% .[-which(.[,"Infection"]=="Mock"),] # this small object shows the mean increase/decrease for affected genera
